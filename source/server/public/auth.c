...
//检测token
void check_token(struct mg_connection *c,char *user,char*token){
    ...
    if(!check_time(data->token,data->token_time)||strcmp(token,data->token)!=0){
        mg_error(c, "error: %d", 401);
    }
    }
}
//检测操作
void check_action(struct mg_connection *c,char*user,char*token,char*control,char*action){
check_token(c,user,token);
int permit=0;
    char *config=get_config();
    cJSON *json_config=cJSON_Parse(config);
