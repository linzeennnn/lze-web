...
void edit_path(string type,menu*last_win){
    ...
    if(cur_path=="default"){
        ...
        open_path_edit_win(nullptr,cur_path,default_path,key);
}
void open_path_edit_win(
    ...
    menu *path_edit_win;
    vector<option*> list={
        new option(text_box("backPar"),[&path_edit_win,scan_path,default_path,key]()
        {open_path_edit_win(path_edit_win,scan_path.parent_path(),default_path,key);}),
        new option(text_box("useDefault"),[&path_edit_win,scan_path,default_path,key]()
        {open_path_edit_win(path_edit_win,default_path,default_path,key);})
    };
 for (const auto& entry : fs::directory_iterator(scan_path)) {
            ...
            if(fileType=="dir"||fileType=="dir_link")
                list.push_back(new option(entry.path().filename(),[&path_edit_win,scan_path,default_path,entry,key]()
        {open_path_edit_win(path_edit_win,entry.path(),default_path,key);}));
        }
